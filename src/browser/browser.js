...
class Browser {
	...
	static async init() {
		...
		if (process.platform=='linux') {
			var service = new firefox.ServiceBuilder(PATH.join(__dirname, '../geckodriver_linux_32')).build();
	...
	navigate(arg) {
		...
		var navprom=new Promise(async res=>{
			...
			var donav = arg.constructor==String
				? ()=>{driver.get(arg);}
				: arg;
		...
		return {
			...
			ready: ()=> 
				new Promise(async res=>{
					var driver=await navprom;
					await this.when('return !document._navaway && (document.readyState=="interactive" || document.readyState=="complete")');
					res();
				}),
			load: ()=>
				new Promise(async res=>{
					var driver=await navprom;
					await this.when('return !document._navaway && document.readyState=="complete"');
					res();
				})
		};
	}
