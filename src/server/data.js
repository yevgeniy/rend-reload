...
const { useMongoDb, useUsers, useStates } = require("./hooks");
...
function updateUsers({ db, users }) {
  ...
  useEffect(() => {
    const a = NIMM_SYNC.onupdate(changes => {
      (changes || []).nimmjoin(users).length > 0 && setChanged(c => c + 1);
    });
...
function updateUser({ db, ...user }) {
  const [comp] = useResetableState(
    ({ username, favorite, ice, imgcount, lastUpdated, reachedBottom }) => {
      useUpdate(() => {
...
function updateImages({ db, images }) {
  ...
  useEffect(() => {
    const a = NIMM_SYNC.onupdate(changes => {
      (changes || []).nimmjoin(images).length && setChanges(c => c + 1);
    });
...
function updateImage({ db, ...img }) {
  const [comp] = useResetableState(
    ({ id, marked, drawn, drawing, seen }) => {
      useUpdate(() => {
        new Promise(res =>
          db
            .collection("images")
            .updateOne({ id }, { $set: { marked, drawn, drawing, seen } }, e =>
              res(e)
            )
        ).then(err => {
...
function loadImagesForUser({ currentUserName, db, setImages }) {
  ...
  if (requested.indexOf(currentUserName) > -1) return;
