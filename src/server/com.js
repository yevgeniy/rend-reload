...
const{useUserImages,useStateImages,useModelStates,useStates,useMarkedImages, useModelImages, useUsers, useModel, useModelUsers, useMongoDb, useImageIds, useSync}=require('./hooks');
...
module.exports=function({datetime, images, setImages, users, setUsers, model, syncUsers, syncImages
...
function setUsersOnModel() {
    ...
    switch (showOptions) {
        case 'only-marked':
            return component(setUsersOnModel_markedOnly, {users,images,syncUsers,modelUsers});
        default:
            return component(setUsersOnModel_all, {users, syncUsers});
    }
}
function setUsersOnModel_all({users,syncUsers}) {    
    useEffect(()=> {
        syncUsers(users);
    },users);
}
function setUsersOnModel_markedOnly({users,images,syncUsers}) {
...
function stripImagesForUsers({users, setImages, setUsers, model, datetime, updateNewImages, setNewimages}){
    if (process.argv.indexOf('--nobrowser')>-1)
        return;
    ...
    if (!running) {
        ...
        var userstorun = 
            ...
            .concat(favoriteusers).concat(otherusers)
...
var c=0;
function getImagesRunner({users, setRunning, setRan, setImages, setUsers, timeIndex, updateNewImages
        , setNewimages
        , ...user }) {
    ...
    useEffect(()=> {
        browser.getImages(
            ...
            var userimages=imageIds[user.username].map(x=>{return {id:x}});
